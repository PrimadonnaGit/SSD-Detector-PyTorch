...
# Data parameters
data_folder = "D:\Datasets\VOCdevkit" # folder with data files
keep_difficult = True # use objects considerd difficult to detect
...
grad_clip = None # clip if gradients are exploding, which may happen at larger batch sizes (somtimes at 32)
...
def main():
    ...
    """
    global epochs_since_improvement, start_epoch, label_map, best_loss, epochs, checkpoint

    # Initialize model or load checkpoint
    if checkpoint is None:
        model = SSD300(n_classes=n_classes)
        # Initialize the optimizer, with twice the default learning rate for biases, as in the original repo
        biases = list()
        not_biases = list()
